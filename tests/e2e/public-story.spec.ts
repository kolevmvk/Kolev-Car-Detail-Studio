...
test("homepage tells the one-car story without template chrome", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.getByRole("link", { name: "Pogledaj prvi slobodan termin" }).first()).toBeVisible();
...
test("reduced motion still exposes before and after headlights", async ({ page }) => {
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.goto("/");

  await expect(page.getByText("Pre", { exact: true })).toBeVisible();
  await expect(page.getByText("Posle", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: /Pritisni i drži/ })).toHaveCount(0);
});
